...
export function classifyTask(task) {
    ...
    if (/(review|审查|评审|检查代码|code review|安全审计|audit|pr\b|pull request)/i.test(text))
        return 'review';
    if (/(写|实现|修复|重构|函数|代码|python|javascript|typescript|node|java|go\b|rust|sql|bug|test|测试用例|接口|api|脚本|implement|fix|refactor|write|code|function|script|build)/i.test(text))
        return 'code';
    if (/(分析|比较|调研|研究|原因|诊断|评估|方案|架构|设计|analy|investigat|debug|diagnos|root cause|why|compare|research|evaluate|design|architect|leak)/i.test(text))
        return 'analysis';
    if (/(文档|报告|说明|总结|readme|release note|教程|doc|summar|explain|tutorial)/i.test(text))
        return 'docs';
    if (/(部署|启动|停止|进程|服务|日志|运维|环境|配置|docker|server|deploy|restart|stop|start|monitor|container|kubernetes|process|log)/i.test(text))
        return 'ops';
    return 'general';
}
