...
function readInjectKeys(entry) {
	try {
		...
		if (fiberInject && typeof fiberInject === "object") return Object.keys(fiberInject).filter((k) => typeof k === "string" && k.length > 0);
	} catch {}
	try {
		const configInject = entry.options.inject;
		if (configInject == null) return [];
		if (Array.isArray(configInject)) return configInject.filter((k) => typeof k === "string" && k.length > 0);
		if (typeof configInject === "object") return Object.keys(configInject).filter((k) => typeof k === "string" && k.length > 0);
	} catch {}
	return [];
}
/**
...
function readParentId(entry, all) {
	try {
		...
		if (!parentFiber) return void 0;
		return all.find((candidate) => candidate !== entry && candidate.fiber === parentFiber)?.id;
	} catch {
		return;
	}
}
		...
		*/
		graph() {
			...
			for (const plugin of plugins) {
				...
				if (plugin.parentId) edges.push({
					from: plugin.parentId,
					to: plugin.id,
					kind: "contains"
				});
			}
			const consumerCount = /* @__PURE__ */ new Map();
			for (const plugin of plugins) for (const key of plugin.injects) {
			...
			for (const [key, count] of consumerCount) nodes.push({
				kind: "service",
				service: {
					id: `service:${key}`,
					name: key,
...
//#endregion
export { TopologyGateway, TopologyGateway as default };
