...
export class ObserveStore {
    ...
    query(filter = {}) {
        ...
        if (filter.agent)
            out = out.filter((e) => e.agent === filter.agent);
        if (filter.since)
            out = out.filter((e) => e.startedAt >= filter.since);
        out = [...out].reverse();
    ...
    stats() {
        ...
        for (const e of this.events) {
            if (e.kind === 'tool.call') {
                ...
                bucket.streams += 1;
                if (typeof e.features?.chunks === 'number')
                    bucket.totalChunks += e.features.chunks;
                if (e.durationMs !== undefined)
                    bucket.durations.push(e.durationMs);
        ...
        const topTools = [...toolByName.entries()]
            ...
            .slice(0, 8);
        const topModels = [...modelByName.entries()]
